...
const AuthGate = ({ onDone }: Props) => {
  ...
  const complete = async (profile: any) => {
    try {
      await setItem(
        "SS_USER_PROFILE",
        JSON.stringify({ ...profile, role: selectedRole })
      );
  ...
  const onSelectRole = (role: UserRole) => {
    ...
    if (role === "student") setMode("studentType");
    else setMode("menu");
  };
    ...
    >
      ...
      >
        <View
          style={[
            styles.centerWrap,
            mode === "signup" && { justifyContent: "flex-start" },
          ]}
        >
          ...
          {mode === "roleSelect" && (
            <View style={styles.card}>
              ...
              <View style={styles.rolesContainer}>
                <RoleButton role="student" icon="school" label="Student" />
                <RoleButton role="mentor" icon="person" label="Mentor" />
                <RoleButton
                  role="guardian"
              ...
              <Text style={styles.terms}>
                Choose your role to get personalized guidance and features
              </Text>
            </View>
          )}
          ...
          {mode === "studentType" && (
            <View style={styles.card}>
              ...
              <View style={{ gap: 10 }}>
                <TouchableOpacity
                  style={[styles.roleButton, { width: "100%" }]}
  ...
  const save = async () => {
    try {
      ...
      const profile = {
        ...base,
        isPwd: true,
        pwdType: PWD_TYPES[typeIdx],
        pwdPercent: PWD_PERCENT[pctIdx],
        role: "student",
      };
  ...
  return (
    <View>
      ...
      <TouchableOpacity
        ...
        onPress={() => setTypeIdx((typeIdx + 1) % PWD_TYPES.length)}
      >
        <MaterialIcons name="arrow-drop-down" size={20} color={NAVY} />
        <Text style={{ marginLeft: 6, color: NAVY, fontWeight: "700" }}>
          {PWD_TYPES[typeIdx]}
        </Text>
      </TouchableOpacity>
      <Text style={{ color: "#374151", marginVertical: 8 }}>
        📊 Disability Percentage
      </Text>
      <TouchableOpacity
        style={styles.inputRow}
        onPress={() => setPctIdx((pctIdx + 1) % PWD_PERCENT.length)}
      >
        <MaterialIcons name="arrow-drop-down" size={20} color={NAVY} />
        <Text style={{ marginLeft: 6, color: NAVY, fontWeight: "700" }}>
          {PWD_PERCENT[pctIdx]}
